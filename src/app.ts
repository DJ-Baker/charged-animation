...
var charScene = new sg.Scene(<HTMLDivElement>document.getElementById("my-character"));
///////////////////////////////////
// Scene 
...
var s5renderFunc = function(t: number) {
	...
	var opMove = function(startT: sg.Vector, startR: sg.Vector, distX: number, distY: number, distZ: number, rotX: number,rotY:number, t: number) {
	...
	var wave = function(waveZ: number, waveC: number, t: number) {
	...
	var firsthandwave = function(startR: sg.Vector, rotZ: number,yTran:number, t: number) {
	...
	var handwave = function(startR: sg.Vector, rotZ: number, t: number) {
		if (armZ > 1 || armZ < -46) {
			tempZ *= -1;
		}
		let waveRate = rotZ/(t*100);
